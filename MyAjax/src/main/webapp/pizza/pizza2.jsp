...
<script type="text/javascript">
	...
	const init=function(){
		try{
			...
			try{			//IE 예전 버전인 경우
			req=new ActiveXObject("Xsxml2.XMLHTTP");
			}catch(e2){
				req=false;
			}
		}
	...
	const getUserInfo=function(){
		...
		let phone=$('#phone').val();
		//alert(phone);
		//서버측에 고객의 연락처 정보를 파라미터로 전달해보자
		//[1]
		req.open('GET','pizzaResult2.jsp?phone='+phone,true);
		//open(요청방식,url,비동기여부)==>요청을 보낼 준비를하는 메서드
		...
		//post방식 요청일 경우 파라미터데이터 (ex=>"phone=111&userid=aaa")를 매개변수로 넣어준다.
	}//----------------------------------
	const updatePage=function(){
		...
		if(req.readyState==4&&req.status==200){
			...
			//alert(res);//XMLdocument객체로 변환됨==>이문서를 파싱해서 필요한 데이터를 추출하자
			...
			let type=$(res).find("addr").attr("type");//속성을 가져올 때 attr("속성명")==>getter
													//attr("type","m2")===>setter
			alert(type);
			if(parseInt(num)==0){
				//비회원인 경우-배달할 주소입력 폼을 보여줌
				$('#nonUser').show(1000);
				$('#userInfo').html("").hide(1000);
				$('#addr').val("").focus();
			}else{
				//회원인 경우=>회원 정보 보여주기
				let str=`<ul class='list-group' style='background:yellow'>
	...
	name="orderF" id="orderF"
	 action="order.jsp" method="POST">
		<div class="form-group">
			<p class="text-info">
			<b>귀하의 전화번호를 입력하세요:</b>
			<input type="text" size="20"name="phone" id="phone" onchange="getUserInfo()"
			  class="form-control"/>
			</p>
			<p class="text-danger">
			<b>
