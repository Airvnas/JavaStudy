...
public class Weka02DecisionTree {
	...
	public Weka02DecisionTree() throws Exception{
		...
		train.setClassIndex(train.numAttributes()-1);//마지막 속성을 target으로 지정
	...
	public void generateModel_Evaluate() throws Exception{
		model=new J48();
		((J48)model).setMinNumObj(10);//노드 당 최소 인스턴스 수 지정=>설정값이 클수록 노드가 줄어들면서
									//나무의 깊이도 낮아진다.
		((J48)model).setUnpruned(false);//가지치기 하려면 false설정
		...
		System.out.println("분류대상 데이터 건수: "+nums);
