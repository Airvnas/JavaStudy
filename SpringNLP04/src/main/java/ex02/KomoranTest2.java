...
//https://docs.komoran.kr/firststep/postypes.html 코모라 품사표
//https://www.shineware.co.kr/products/komoran/ 코모라 문장분석
@Log4j
public class KomoranTest2 {
	...
	public static void main(String[] args) {
		...
		//getTokenList(): token형태로 결과를 담는데 이 토큰에는 형태소가 문자열의 어느위치에서 생성되는지 끝나는지,
		//형태소 분석결과 등을 반환한다.
		...
		log.info(info);
		//pos tagging이 붙어있는 텍스트 형태로 반환
		
		log.info("===3.getMorpheByTags====================");
		List<String> arr=res.getMorphesByTags("NNG","NNP","NNB");//명사들만 추출
		log.info(arr);
		
		log.info(res.getMorphesByTags("VV"));
	}
