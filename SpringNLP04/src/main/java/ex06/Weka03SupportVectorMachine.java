...
public class Weka03SupportVectorMachine {
	...
	public void generateModel_Evaluate() throws Exception{
		...
		model.buildClassifier(train);//학습진행
	...
	public void predictOne(String pclass,double age,String sex) throws Exception{
		Attribute attrPclass=new Attribute("pclass",Arrays.asList("1","2","3"),0);//속성명,List(속성),index
		...
		Instances intstances=new Instances("SMO",attrs,0);
		intstances.setClassIndex(1);
		
		//@data
		Instance obj=new DenseInstance(4);//4:속성 개수
		obj.setValue(attrPclass, pclass);
		obj.setValue(attrSex, sex);
		obj.setValue(attrAge, age);
		obj.setDataset(intstances);
