...
<div class = "row">
	...
	<c:if test="${pList ne null and not empty pList }">
		<c:forEach var="pd" items="${pList}">
	      ...
	      <!-- column을 12개 기준으로 3개씩 나타냄  -->
	         <a href="prodDetail?pnum=${pd.pnum}">
	         <c:if test="${pd.pimage1 ne null}">
	         <img src = "resources/product_images/${pd.pimage1}" class = "img img-fluid"
	         		style="width:90%;margin:auto;height:50%">
	         </c:if>
	         <c:if test="${pd.pimage1 eq null}">
	         <img src = "resources/product_images/noimage.png" class = "img img-fluid"
	         style = "height:220px">
	         </c:if>
	         </a>
	         <br><br>
