...
public class FileController {
	...
	public String fileUpload(Model m,@RequestParam("name") String name,@RequestParam("mfilename") MultipartFile mfilename) {
		...
		if(!mfilename.isEmpty()) {//null로 체크x
	...
	//[2] MultipartHttpServletRequest 이용하는 방법
	//=>동일한 파라미터 명으로 여러개의 파일을 업로드하는 경우에 유용하다.
	@PostMapping("/fileUp2")
	...
	/*데이터와 함께 헤더 상태 메시지를 전달할 때 사용한다.
	 * Http Header를 다뤄야할 경우 ResponseEntity를 통해 헤더 정보나 데이터를 전달할 수 있다.
	 * HttpEntity를 상속받아 구현한 클래스
	 * -ResponseEntity:요청 헤더정보
	 * -ResponseEntity(HttpStatus, HttpHeaders, HttpBody):응담 헤더정보 + 응답 데이터
	 * 
