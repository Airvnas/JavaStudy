...
create table spring_board(
    ...
    filename varchar2(500),--첨부파일명[uuid_a.txt]==>물리적 파일명(실제 서버에 업로드된 파일명)
    originFilename varchar2(500),--원본 파일명[a.txt]논리적 파일명(사용자가 아는 파일명)
    filesize number(8),--첨부파일 크기
    refer number(8), --글 그룹번호[답변형 게시판일때 필요한 컬럼]
    lev number(8),--답변 레벨[답변형 게시판일때 필요한 컬럼]
    sunbun number(8) --같은 글 그룹내에 순서 정렬할 때 필요한 컬럼[답변형 게시판일때 필요한 컬럼]
);
