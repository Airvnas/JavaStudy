...
//static import문을 이용하여 클래스 명을 생략하고 사용
//eq(),gt(),lt(),gte()....
...
public class TestMongoPOJO {
	...
	//Bson문서를 Java Pojo객체에 매핑하는 메서드==> 코덱 레지스트리가 필요함
		/*POJO에 CodecRegistry변환을 처리할 코덱을 포함하도록 구성=>
		 * 다음에는 기본 코덱 레지스트리 PojoCodecProvider를 자동으로 생성하도록 구성된 레지스트리와 결합	
		 * */
	...
	public void insertOne() {
		...
		PostVO vo= new PostVO(null,"Kim","오늘도 수고했어요","2022-12-13");
		mcol.insertOne(vo);
	...
	public void insertMany() {
		...
		List<PostVO> postsArr=Arrays.asList(
				...
				new PostVO(null,"James","감기조심하세요","2022-12-14"),
				new PostVO(null,"King","^^.","2022-12-14"));
	...
	public void updateOne() {
		System.out.println("수정할 글의 작성자 명을 입력하세요: ");
		String name=sc.nextLine();
	...
	public void updateMany() {
		System.out.println("수정하지 않을 글의 작성자 명을 입력하세요: ");
		String name=sc.nextLine();
		System.out.println("수정할 글 제목: ");
		String title=sc.nextLine();
		System.out.println("수정할 글의 작성일[yyyy-MM-dd]: ");
		...
		mcol=mdb.getCollection(table,PostVO.class);
		//UpdateResult res=mcol.updateMany(not(eq("author",name)), combine(set("title",title)));
		UpdateResult res=mcol.updateMany(ne("author",name), combine(set("title",title)));
		long n=res.getModifiedCount();
		System.out.println(n+"개의 도큐먼트 수정");
	}//--------------------------------------------
	
	public void replaceOne() {
		mcol=mdb.getCollection(table,PostVO.class);
		PostVO vo=new PostVO(null,"김길동","replaceOne으로 Posts내용을 변경","2022-12-14");
